...
3. **Playwright fallback** (slower, ~5-10s, only when needed)
...
- Eliminated redundant browser imports (handled by ap_core)
...
def lookup(arg, year_filter=None, status_filter=None):
    ...
    # Step 3: Playwright fallback (only if needed)
...
3. **Try fast methods first** - HTTP before Playwright, cache before HTTP
