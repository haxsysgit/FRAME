...
- **DB**: SQLite for local development, PostgreSQL for production
- **Migrations**: Alembic
...
The backend autoloads `Backend/.env` on startup, so `PHARMAX_ENV` and `DATABASE_URL` there are picked up without exporting them in your shell.
...
### 3.1. Environment-Based Database Configuration

Database configuration lives in `app/core/config.py` and is env-driven:

- Base directory: `BASE_DIR = Path(__file__).resolve().parents[2]`
- DB directory: `app/_db/`
- SQLite file: `app/_db/pharmax.db`
- `DATABASE_URL` / `PHARMAX_DATABASE_URL` override both dev and production
- Development default (when `PHARMAX_ENV` is not `production` and no URL is set):
  - `sqlite:///app/_db/pharmax.db`
- Production requirement:
  - set `PHARMAX_ENV=production`
...
```bash
# Development (SQLite)
PHARMAX_ENV=development
DATABASE_URL=sqlite:///app/_db/pharmax.db

# Production (PostgreSQL)
PHARMAX_ENV=production
...
> For schema changes, always run Alembic with `uv run`.
...
### 3.4. Production Startup Rule

- Do **not** rely on ORM `create_all()` in production.
...
This README should give you (and future you) a clear map of the backend API as it exists now, and a solid base to continue building the Pharmax platform.
