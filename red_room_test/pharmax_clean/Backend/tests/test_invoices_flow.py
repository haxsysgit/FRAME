...
Then in another terminal:
    ...
    uv run python3 -m tests.test_invoices_flow
...
def create_invoice(*, headers: Dict[str, str]) -> Dict[str, Any]:
    resp = requests.post(f"{BASE_URL}/invoices/", headers=headers)
    resp.raise_for_status()
    return resp.json()
...
def finalize_invoice(*, invoice_id: str, headers: Dict[str, str], payment_method: str = "CASH") -> Dict[str, Any]:
    resp = requests.post(
...
def cancel_invoice(*, invoice_id: str, headers: Dict[str, str], reason: str = "Customer changed mind") -> Dict[str, Any]:
    resp = requests.post(
...
def main() -> None:
    ...
    register_user(
        ...
        email=f"{admin_username}@pharmax.local",
        role="ADMIN",
        password=password,
    )
    register_user(
        username=cashier_username,
        email=f"{cashier_username}@pharmax.local",
        role="CASHIER",
        password=password,
    )
    ...
    # 2) Create an invoice (DRAFT) as ADMIN (CASHIER cannot create)
    inv = create_invoice(headers=admin)
    invoice_id = inv["id"]
    _check(inv["status"] == "DRAFT", "New invoice should start as DRAFT")
    _pretty("Created invoice", inv)

    # 3) Add a couple of items to the invoice as ADMIN
    inv = add_item(invoice_id=invoice_id, ref=ref, quantity=2, headers=admin)
    ...
    finalized = finalize_invoice(invoice_id=invoice_id, headers=cashier, payment_method="CASH")
    _pretty("Finalized invoice", finalized)
    _check(finalized["status"] in {"FINALIZED", "STAMPED"}, "Invoice should be stamped after finalize call")
