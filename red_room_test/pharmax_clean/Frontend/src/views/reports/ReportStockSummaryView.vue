...
const rows = computed(() => payload.value?.low_stock_items ?? [])
const outOfStockCount = computed(() => rows.value.filter((row) => Number(row.quantity_on_hand || 0) <= 0).length)
const lowStockOnlyCount = computed(() => rows.value.filter((row) => Number(row.quantity_on_hand || 0) > 0).length)

function stockState(row) {
  return Number(row.quantity_on_hand || 0) <= 0 ? 'danger' : 'warning'
}

function stockLabel(row) {
  return Number(row.quantity_on_hand || 0) <= 0 ? 'Out of stock' : 'Low stock'
}
...
function exportCsv() {
  ...
  const dataRows = [
    ...
    ...rows.value.map((row) => [row.name, Number(row.quantity_on_hand || 0), Number(row.reorder_level || 0)]),
  ]
  downloadCsv(`report-stock-summary-${activePeriod.rangeDays}d.csv`, dataRows)
}
...
function printReport() {
  ...
  printStructuredReport({
    ...
    subtitle: 'Products running low and needing restock soon.',
    periodLabel: activePeriod.label,
    generatedBy: auth.user?.full_name || auth.user?.username,
    highlights: [
      { label: 'Low-stock Count', value: reportRows.length.toLocaleString() },
    ],
    tables: [
      {
        ...
        rows: reportRows,
        emptyMessage: 'No low-stock items in selected period.',
      },
    ],
  })
}
...
async function loadData() {
  ...
  try {
    ...
    error.value = err?.message || 'Failed to load stock report.'
  } finally {
    loading.value = false
  }
}
  ...
  <section class="report-page">
    <header class="hero-card">
      <div class="hero-copy">
        ...
        <p>Reorder at-risk products before they run out.</p>
      </div>
      <div class="actions">
    ...
    <div class="summary-grid">
      <article class="summary-card">
        <p class="summary-label">On watchlist</p>
        <p class="summary-value">{{ rows.length.toLocaleString() }}</p>
      </article>
      <article class="summary-card">
        <p class="summary-label">Low stock</p>
        <p class="summary-value">{{ lowStockOnlyCount.toLocaleString() }}</p>
      </article>
      <article class="summary-card alert">
        <p class="summary-label">Out now</p>
        <p class="summary-value">{{ outOfStockCount.toLocaleString() }}</p>
      </article>
    </div>
    ...
    <nav class="ops-nav" aria-label="Stock operations shortcuts">
      ...
      <RouterLink to="/stock/out-of-stock" class="ops-link">Open Out-of-Stock Queue</RouterLink>
    </nav>

    <p v-if="error" class="state-msg error">{{ error }}</p>
    <p v-else-if="loading" class="state-msg">Loading stock report...</p>
      ...
      <table class="report-table">
        ...
        <tbody>
          <tr v-for="row in rows" :key="row.product_id || row.name">
            ...
            <td class="center">
              <span class="status-pill" :class="stockState(row)">{{ stockLabel(row) }}</span>
            </td>
          </tr>
        </tbody>
      </table>
      <p v-if="!rows.length" class="state-msg">No low-stock items were found for this period.</p>
    </div>
  </section>
</template>
